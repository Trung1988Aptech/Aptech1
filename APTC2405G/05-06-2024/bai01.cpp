...
void load(Person* people, const char *filename)
{
    ...
    int count = size / sizeof(Person);
    fseek(inputFile, 0, SEEK_SET); // Quay lại đầu file

    fread(people, sizeof(Person), count, inputFile);
    fclose(inputFile); // Đóng file
}
void analyze(Person *people, int count)
{
    map<string, int> dictionary;

    for (int i = 0; i < count; i++) {
        Person eachPerson = people[i];
        string nationality = eachPerson.nationality;  // Chuyển char[] sang std::string
        /*
        // Kiểm tra xem quốc tịch này đã tồn tại trong map chưa
        if (dictionary.find(nationality) == dictionary.end()) {
            // Nếu chưa có, khởi tạo giá trị đếm là 1
            dictionary[nationality] = 1;
        } else {
            // Nếu đã có, tăng giá trị đếm lên
...
int main()
{
    ...
    while (choice != 7)
    {
        ...
        case 6:
            cout << "6. Open" << endl;
            people2 = new Person[numberOfPeople];
            //people2 = (Person *)malloc(numberOfPeople * sizeof(Person));
            load(people2, fileName);   
            display(people2, numberOfPeople);           
            break;
        case 7:
            cout << "7. Exit" << endl;
